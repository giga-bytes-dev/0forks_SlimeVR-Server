...
import { CanvasTexture, PerspectiveCamera, Vector3 } from 'three';
...
const FaceMaterial = ({
  ...
  const texture = useMemo(() => {
    ...
    context.fillText(faces[index].toUpperCase(), canvas.width / 2, canvas.height / 2 + 20);
...
function SceneRenderer({ x, y, z, w }: { x: number, y: number, z: number, w: number }) {
  return (
    <Canvas
      className='container'
      style={{ height: 200, background: 'transparent'}}
      onCreated={({ camera, gl, scene }) => { (camera as PerspectiveCamera).fov = 60 }}
    >
...
export function IMUVisualizerWidget({ tracker }: { tracker: TrackerDataT }) {
  ...
  const quat = useReferenceAdjustedRotationNoMountingQuat();

  return (
    <div className="bg-background-70 flex flex-col p-3 rounded-lg gap-2">
      <Typography variant="section-title">Rotation</Typography>

      <div className="flex justify-between">
        <Typography color="secondary">Raw</Typography>
        <Typography>
          {formatVector3(useRotation(), 2)}
        </Typography>
      </div>

      <div className="flex justify-between">
        <Typography color="secondary">Adjusted</Typography>
        <Typography>
          {formatVector3(useReferenceAdjustedRotation(), 2)}
        </Typography>
      </div>

      { !enabled && <Button variant='secondary' onClick={() => setEnabled(true)}>Preview</Button> }
      { enabled && <SceneRenderer x={quat.x} y={quat.y} z={quat.z} w={quat.w}></SceneRenderer> }
    </div>
  );
}
